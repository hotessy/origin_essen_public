...
shows origin_essentialism3: "\<lfloor>(\<^bold>\<forall>x. \<^bold>\<forall>y. \<^bold>\<forall>z. \<^bold>\<forall>p. (\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p'))))))\<rfloor>"

proof (rule allI)
  fix w 
  show "(\<^bold>\<forall>x. \<^bold>\<forall>y. \<^bold>\<forall>z. \<^bold>\<forall>p. (\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p')))))) w"
  proof (rule allI)
    fix x
    show "(\<^bold>\<forall>y. \<^bold>\<forall>z. \<^bold>\<forall>p. (\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p')))))) w"
    proof (rule allI)
      fix y 
      show "(\<^bold>\<forall>z. \<^bold>\<forall>p. (\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p')))))) w"
      proof  (rule allI)
        fix z 
        show "(\<^bold>\<forall>p. (\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p')))))) w"
        proof (rule allI)
          fix p 
          show "((\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p)) \<^bold>\<rightarrow> \<^bold>\<not>(\<^bold>\<diamond>(\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p')))))) w"
          proof  (rule impI)
            ...
            proof  (rule notI)
              ...
              from antecedent have "((\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p))) w" by simp
              then have non_overlapping: "(\<^bold>\<not>(y \<^bold>=\<^sup>L z)) w" by (rule conjE)
              have table_x_from_y: "(P x y p) w" using `((\<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> (P x y p))) w` by (rule conjE)
              ...
              then obtain v where v: "(w r v) \<and> (((\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p'))))) v)" by (rule exE)
              then have "w r v" by  (rule conjE)
              have  "((\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p'))))) v" 
                using `(w r v) \<and> (((\<^bold>\<forall>p'. P x z p' \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p'))))) v)` by (rule conjE)
              then have "((P x z p \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p))))) v" by (rule allE)
              then have possibility: "(\<^bold>\<diamond>(P x z p \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p))))) w" 
                using `w r v`by auto
              ...
              then obtain u where u: "(w r u) \<and> ((((P x y p) \<^bold>\<and> (\<^bold>\<exists>x'. P x' z p \<^bold>\<and> (\<^bold>\<forall>x''.(\<^bold>\<not>(x' \<^bold>=\<^sup>L x'') \<^bold>\<rightarrow> \<^bold>\<not>(P x'' z p)))))) u)" by (rule exE)
              then  have "((((P x y p) \<^bold>\<and> (\<^bold>\<exists>x'. P x' z p \<^bold>\<and> (\<^bold>\<forall>x''.(\<^bold>\<not>(x' \<^bold>=\<^sup>L x'') \<^bold>\<rightarrow> \<^bold>\<not>(P x'' z p)))))) u)" by (rule conjE)
              ...
              have "(w r u)" 
                using `(w r u) \<and> ((((P x y p) \<^bold>\<and> (\<^bold>\<exists>x'. P x' z p \<^bold>\<and> (\<^bold>\<forall>x''.(\<^bold>\<not>(x' \<^bold>=\<^sup>L x'') \<^bold>\<rightarrow> \<^bold>\<not>(P x'' z p)))))) u)` by (rule conjE)
              ...
              then have "((P x' z p \<^bold>\<and> (\<^bold>\<forall>t.(\<^bold>\<not>(x' \<^bold>=\<^sup>L t) \<^bold>\<rightarrow> \<^bold>\<not>(P t z p)))) \<^bold>\<rightarrow> (x'\<^bold>=\<^sup>Lx)) u" 
                using `w r u` by auto
