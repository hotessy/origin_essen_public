          ...
          proof (rule allI)
            ...
            proof (rule impI)
              ...
              proof (rule notI)
                ...
                have compossibilty4_ante: "(N x y p k \<^bold>\<and> \<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<and> \<^bold>\<diamond>(\<^bold>\<exists>t. N t z p k)) w"
                  using antecedent and sufficiency4_ante by auto
